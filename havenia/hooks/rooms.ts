...
export const useRooms = (params: {
  ...
  useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ data: Room[]; from?: string; to?: string; nights?: number }>(
        "/rooms",
        { params }
      );
      return data.data;
...
export const useMyReservations = () =>
  useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ data: Reservation[] }>(`/rooms/reservations`);
      return data.data;
    },
  });
...
export const useMyReservation = (id?: string) =>
  useQuery({
    ...
    queryFn: async () => {
      const { data } = await api.get<{ reservation: Reservation }>(`/rooms/reservations/${id}`);
...
export const useCreateReservation = () => {
  ...
  return useMutation({
    mutationFn: async (body: { room_id: string; start_date: string; end_date: string }) => {
      const { data } = await api.post<{ reservation: Reservation }>(`/rooms/reservations`, body);
      return data.reservation;
    },
    onSuccess: () => {
